...
public class Chunk : MonoBehaviour
{
    ...
    public void InitMesh() {
        ...
        int totalLength = 2 * settings.chunkLength * settings.viewDistance + settings.chunkLength;
        for (int x = dx; x < dx + settings.chunkLength; x++) {
            for (int z = dz; z < dz + settings.chunkLength; z++) {
                y = ChunksGen.SurfaceYPos[(x / settings.chunkLength + 1) * dz];
                if (Blocks[x, y, z] == BlockType.Air) continue;
                int faces = 0;
                //top
                if (y == settings.chunkHeight - 1 || !BlockFromMesh(x, y + 1, z).OpaqueDown) {
    ...
    public void RefreshMesh() {
        ...
        int totalLength = 2 * settings.chunkLength * settings.viewDistance + settings.chunkLength;
        for (int x = dx; x < dx + settings.chunkLength; x++) {
            for (int y = 0; y < ChunksGen.SurfaceYMax + 20; y++) {
                for (int z = dz; z < dz + settings.chunkLength; z++) {
